...
struct rdma_buf {
    ...
    // public:
        ...
        void start(uint64_t h_address, size_t user_size){
            ...
            // allocate memory on gpu and host
            alloc_memory((T *) host_address, (T *) gpu_address);
            printf("host_address: %p Address_Offset: %p\n", host_address, Address_Offset);
        ...
        T& operator[](uint64_t index) {
            ...
            // #ifdef  __CUDA_ARCH__
                ...
                // __syncthreads();
                    ...
                    if(index < 0 || index >= size/sizeof(T) || che < 0 || che >= tlb_size) printf("index: %llu Invalid index\n", index);
                    ...
                    if(d_TLB[che].state == 0 || d_TLB[che].state == 5){ // page completely on cpu or dirty on cpu
        ...
        void rvalue(size_t index, T i){
            ...
            T *tmp_array =  (T *)d_TLB[che].device_address;
            tmp_array[index%request_size] = i;
