...
struct connection {
  ...
  struct ibv_mr *rdma_remote_mr[N_8GB_Region];
...
void send_qp_info(void *context){
  ...
  printf("Function name: %s, line number: %d RDMA_BUFFER_SIZE: %llu\n", __func__, __LINE__, RDMA_BUFFER_SIZE);
  /*modify qp to init*/
  for(int i = 0; i < N_QPs; i++){
...
void register_memory(struct connection *conn)
{
  ...
  conn->rdma_local_region = malloc((RDMA_BUFFER_SIZE/4) * sizeof(int));
  conn->rdma_remote_region = (void *) malloc(RDMA_BUFFER_SIZE);

  if (conn->rdma_remote_region == NULL) {
    printf("Failed to allocate remote memory for connection struct\n");
    exit(-1);
  }
  ...
  TEST_Z(conn->rdma_local_mr = ibv_reg_mr(
    ...
    RDMA_BUFFER_SIZE/4*sizeof(int), 
    IBV_ACCESS_LOCAL_WRITE));
    // ((s_mode == M_WRITE) ? 0 : IBV_ACCESS_LOCAL_WRITE)));


  for(int index = 0; index < N_8GB_Region; index++){
    TEST_Z( conn->rdma_remote_mr[index] = ibv_reg_mr(
            s_ctx->pd, 
            conn->rdma_remote_region + index*Region_Size, 
            Region_Size, 
            IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_READ));
            ...
            conn->rdma_remote_mr[index]->addr, conn->rdma_remote_mr[index]->rkey, conn->rdma_remote_mr[index]->length);
...
void send_message(struct connection *conn)
{
  ...
  sge.addr = (uintptr_t) &pool; // conn->send_msg;
  sge.length = sizeof(struct MemPool);
  sge.lkey = pool_mr->lkey;
