...
#include "../../include/runtime_prefetching_2gpu.h"
// #include "../../include/runtime_eviction_2gpu.h"
...
void mvtCuda_2gpu(DATA_TYPE* a, DATA_TYPE* &x1, DATA_TYPE* &x2, DATA_TYPE* y_1, DATA_TYPE* y_2)
{
    ...
    cudaSetDevice(1);
	mvt_kernel1<<<grid,block, 0, stream1>>>(a_gpu1,x1_gpu1, y_1_gpu1);
	mvt_kernel2<<<grid,block, 0, stream1>>>(a_gpu1,x2_gpu1, y_2_gpu1, y_2_gpu1);
    ...
    check_cuda_error(cudaMemcpy(x1, x1_gpu0, sizeof(DATA_TYPE) * N, cudaMemcpyDeviceToHost));   
	check_cuda_error(cudaMemcpy(x2, x2_gpu0, sizeof(DATA_TYPE) * N, cudaMemcpyDeviceToHost)); 
...
{   
    ...
    bool rdma_flag = true;
    cudaError_t ret1;
    struct context_2gpu *s_ctx = (struct context_2gpu *)malloc(sizeof(struct context_2gpu));
    if(rdma_flag){
        ...
        s_ctx->gpu_buf1_size = 16*1024*1024*1024llu; // N*sizeof(int)*3llu;
        s_ctx->gpu_buf2_size = 16*1024*1024*1024llu; // N*sizeof(int)*3llu;
        ...
        int ret = local_connect_2gpu(mlx_name, s_ctx);

        ret = prepare_post_poll_content_2gpu(s_ctx, &post_cont, &poll_cont, &post_cont2, \
    ...
    if(rdma_flag){
        ...
        cudaFree(s_ctx->gpu_buffer1);
        cudaFree(s_ctx->gpu_buffer2);
    }
