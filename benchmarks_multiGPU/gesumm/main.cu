          ...
          rdma_buf<DATA_TYPE> *x_gpu)
{
    ...
    {
		x[i] = ((DATA_TYPE) i) / N1;
		x_gpu->host_buffer[i] = x[i]; // ((DATA_TYPE) i) / N1;
      	printf("A[%d]: %f\n", i*N1, A[i*N1]);
		for (j = 0; j < N1; j++)
		{
			A[i*N1 + j] = ((DATA_TYPE) i*j) / N1;
			B[i*N1 + j] = ((DATA_TYPE) i*j) / N1;
			A_gpu->host_buffer[i*N1 + j] = A[i*N1 + j]; // ((DATA_TYPE) i*j) / N1;
			B_gpu->host_buffer[i*N1 + j] = B[i*N1 + j] ; // ((DATA_TYPE) i*j) / N1;
		}
    }
}
...
void compareResults(DATA_TYPE* y, rdma_buf<DATA_TYPE> *y_outputFromGpu)
{
	...
	for (i=0; i<(N1); i++) 
	{
		if (percentDiff(y[i], y_outputFromGpu->host_buffer[i]) > PERCENT_DIFF_ERROR_THRESHOLD) 
		{
            // printf("i: %llu y[i]: %f, y_outputFromGpu->host_buffer[i]: %f\n", i, y[i], y_outputFromGpu->host_buffer[i]);
			fail++;
		}
	}
                               ...
                               rdma_buf<DATA_TYPE> *y, rdma_buf<DATA_TYPE> *tmp)
{
	...
	if (i < N1)
	{
		...
		{	
			...
			// (*tmp).rvalue(i, (*tmp)[i] + a_*x_);
			// (*y).rvalue(i, (*y)[i] + b_*x_);
            tmp_var += a_*x_;
            y_var += b_*x_;
            
            // if(j == 1){
            //     printf("index: %llu (*a)[index]: %f, (*b)[index]: %f\n", index, a_, b_);
            //     printf("i: %llu tmp_: %f, y_: %f\n", i, (*tmp)[i], (*y)[i]);
            // }
		}
		// y[i] = ALPHA * tmp[i] + BETA * y[i];
...
{   
    ...
    if (A == NULL || B == NULL || x == NULL || A_gpu->host_buffer == NULL || B_gpu->host_buffer == NULL || x_gpu->host_buffer == NULL) {
