...
// #include "../../include/runtime_eviction_2gpu_2nic.h"
...
void mvtCuda_original(DATA_TYPE* a, DATA_TYPE* &x1, DATA_TYPE* &x2, DATA_TYPE* y_1, DATA_TYPE* y_2)
{
...
void mvtCuda_2gpu(DATA_TYPE* a, DATA_TYPE* &x1, DATA_TYPE* &x2, DATA_TYPE* y_1, DATA_TYPE* y_2)
{
    ...
    memcpy(x1_cpu, x1, N*sizeof(DATA_TYPE));
    memcpy(x2_cpu, x2, N*sizeof(DATA_TYPE));
    printf("Allocating on GPU for mvtCuda_2gpu\n");
	
    cudaSetDevice(0);
    check_cuda_error(cudaMallocManaged(&a_gpu0, sizeof(DATA_TYPE) * N * N));
	check_cuda_error(cudaMalloc(&x1_gpu0, sizeof(DATA_TYPE) * N));   
	check_cuda_error(cudaMalloc(&x2_gpu0, sizeof(DATA_TYPE) * N));   
	check_cuda_error(cudaMalloc(&y_1_gpu0, sizeof(DATA_TYPE) * N));  
	check_cuda_error(cudaMalloc(&y_2_gpu0, sizeof(DATA_TYPE) * N));  
    
    printf("Initializing finished on GPU. Transferring to GPU 0...\n");
    cudaSetDevice(0);
    // check_cuda_error(cudaMemcpy(a_gpu0, a, sizeof(DATA_TYPE) * N * N, cudaMemcpyHostToDevice));
    memcpy(a_gpu0, a, sizeof(DATA_TYPE) * N * N);
    check_cuda_error(cudaMemAdvise(a_gpu0, sizeof(DATA_TYPE) * N * N, cudaMemAdviseSetReadMostly, 0));
    check_cuda_error(cudaMemAdvise(a_gpu0, sizeof(DATA_TYPE) * N * N, cudaMemAdviseSetReadMostly, 1));
	check_cuda_error(cudaMemcpy(y_1_gpu0, y_1, sizeof(DATA_TYPE) * N, cudaMemcpyHostToDevice));  
	check_cuda_error(cudaMemcpy(y_2_gpu0, y_2, sizeof(DATA_TYPE) * N, cudaMemcpyHostToDevice));  
    check_cuda_error(cudaMemcpy(x1_gpu0, x1, sizeof(DATA_TYPE) * N, cudaMemcpyHostToDevice));  
	check_cuda_error(cudaMemcpy(x2_gpu0, x2, sizeof(DATA_TYPE) * N, cudaMemcpyHostToDevice));  

    cudaSetDevice(1);
    // check_cuda_error(cudaMallocManaged(&a_gpu1, sizeof(DATA_TYPE) * N * N));
	check_cuda_error(cudaMalloc(&x1_gpu1, sizeof(DATA_TYPE) * N));   
	check_cuda_error(cudaMalloc(&x2_gpu1, sizeof(DATA_TYPE) * N));   
	check_cuda_error(cudaMalloc(&y_1_gpu1, sizeof(DATA_TYPE) * N));  
	check_cuda_error(cudaMalloc(&y_2_gpu1, sizeof(DATA_TYPE) * N));  
    
    printf("Initializing finished on GPU. Transferring to GPU 1...\n");
    cudaSetDevice(1);
    // check_cuda_error(cudaMemcpy(a_gpu1, a, sizeof(DATA_TYPE) * N * N, cudaMemcpyHostToDevice));
    // memcpy(a_gpu1, a, sizeof(DATA_TYPE) * N * N);
    // check_cuda_error(cudaMemAdvise(a_gpu1, sizeof(DATA_TYPE) * N * N, cudaMemAdviseSetReadMostly, 1));
	check_cuda_error(cudaMemcpy(y_1_gpu1, y_1, sizeof(DATA_TYPE) * N, cudaMemcpyHostToDevice));  
	check_cuda_error(cudaMemcpy(y_2_gpu1, y_2, sizeof(DATA_TYPE) * N, cudaMemcpyHostToDevice));  
    check_cuda_error(cudaMemcpy(x1_gpu1, x1, sizeof(DATA_TYPE) * N, cudaMemcpyHostToDevice));  
	check_cuda_error(cudaMemcpy(x2_gpu1, x2, sizeof(DATA_TYPE) * N, cudaMemcpyHostToDevice));  

    oversubs(0.31, sizeof(DATA_TYPE) * N * N);
    ...
    cudaSetDevice(0);
	mvt_kernel1<<<grid,block, 0, stream0>>>(a_gpu0, x1_gpu0, y_1_gpu0);
	mvt_kernel2<<<grid,block, 0, stream0>>>(a_gpu0, x2_gpu0, y_2_gpu0, y_2_gpu0);

    cudaSetDevice(1);
	mvt_kernel1<<<grid,block, 0, stream1>>>(a_gpu0, x1_gpu1, y_1_gpu1);
	mvt_kernel2<<<grid,block, 0, stream1>>>(a_gpu0, x2_gpu1, y_2_gpu1, y_2_gpu1);

    // Synchronize streams to ensure all kernels complete
    cudaSetDevice(0);
    ...
    // cudaSetDevice(0);
	// mvt_kernel1<<<grid,block>>>(a_gpu0,x1_gpu0, y_1_gpu0);
	// mvt_kernel2<<<grid,block>>>(a_gpu0,x2_gpu0, y_2_gpu0, y_2_gpu0);
    ...
    check_cuda_error(cudaMemcpy(x1, x1_gpu1, sizeof(DATA_TYPE) * N, cudaMemcpyDeviceToHost));   
	check_cuda_error(cudaMemcpy(x2, x2_gpu1, sizeof(DATA_TYPE) * N, cudaMemcpyDeviceToHost)); 

    cudaSetDevice(0);
    check_cuda_error(cudaFree(a_gpu0));
    check_cuda_error(cudaFree(x1_gpu0));
    check_cuda_error(cudaFree(x2_gpu0));
    check_cuda_error(cudaFree(y_1_gpu0));
    check_cuda_error(cudaFree(y_2_gpu0));

    cudaSetDevice(1);
    // check_cuda_error(cudaFree(a_gpu1));
    check_cuda_error(cudaFree(x1_gpu1));
    check_cuda_error(cudaFree(x2_gpu1));
    check_cuda_error(cudaFree(y_1_gpu1));
    check_cuda_error(cudaFree(y_2_gpu1));
...
__global__ void mvt_kernel1_rdma(rdma_buf<DATA_TYPE> *a, DATA_TYPE *x1, DATA_TYPE *y_1, int gpu)
{
	size_t i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void mvt_kernel2_rdma(rdma_buf<DATA_TYPE> *a, DATA_TYPE *x2, DATA_TYPE *y_2, DATA_TYPE *a_gpu, int gpu)
{
	size_t i = blockIdx.x * blockDim.x + threadIdx.x;
...
void mvtCuda_rdma_original(DATA_TYPE* a, DATA_TYPE* &x1, DATA_TYPE* &x2, DATA_TYPE* y_1, DATA_TYPE* y_2)
{
    // DATA_TYPE* a_gpu;
	DATA_TYPE* x1_gpu;
	DATA_TYPE* x2_gpu;
    ...
    rdma_buf<DATA_TYPE> *rdma_a_gpu0;
    rdma_buf<DATA_TYPE> *rdma_a_gpu1;

    cudaSetDevice(0);
    check_cuda_error(cudaMallocManaged((void **) &rdma_a_gpu0, sizeof(rdma_buf<DATA_TYPE>)));
    cudaSetDevice(1);
    check_cuda_error(cudaMallocManaged((void **) &rdma_a_gpu1, sizeof(rdma_buf<DATA_TYPE>)));
    
    cudaSetDevice(0);
    rdma_a_gpu0->start(N*N*sizeof(DATA_TYPE), 0, NULL);
    cudaSetDevice(1);
    rdma_a_gpu1->start(N*N*sizeof(DATA_TYPE), 1, NULL /*(void *) rdma_a_gpu0->local_buffer*/);

    cudaSetDevice(0);
    for(size_t i = 0; i < N*N; i++){
        rdma_a_gpu0->local_buffer[i] = a[i];
    }

    cudaSetDevice(1);
    for(size_t i = 0; i < N*N; i++){
        rdma_a_gpu1->local_buffer[i] = a[i];
    }
	...
	mvt_kernel1_rdma<<<grid,block>>>(rdma_a_gpu0, x1_gpu, y_1_gpu, 0);
    ret = cudaDeviceSynchronize();
    printf("ret: %d cudaGetLastError: %d for kernel1\n", ret, cudaGetLastError());
    cudaSetDevice(0);
    mvt_kernel2_rdma<<<grid,block>>>(rdma_a_gpu0, x2_gpu, y_2_gpu, y_2_gpu, 0);
	ret = cudaDeviceSynchronize();
    printf("ret: %d cudaGetLastError: %d\n", ret, cudaGetLastError());
	auto end = std::chrono::steady_clock::now();
    long duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    
    printf("Elapsed time for GPU 0 RDMA in milliseconds: %li ms.d\n\n", duration);
    cudaSetDevice(0);
    ret = cudaDeviceSynchronize();
    // print_retires<<<1,1>>>();
    ...
    cudaSetDevice(1);
    ret = cudaDeviceSynchronize();
	mvt_kernel1_rdma<<<grid,block>>>(rdma_a_gpu1, x1_gpu1, y_1_gpu1, 1);
    ret = cudaDeviceSynchronize();
    printf("GPU1 ret: %d cudaGetLastError: %d for kernel1\n", ret, cudaGetLastError());
    cudaSetDevice(1);
    mvt_kernel2_rdma<<< grid, block >>>(rdma_a_gpu1, x2_gpu1, y_2_gpu1, y_2_gpu1, 1);
	ret = cudaDeviceSynchronize();
    printf("ret: %d cudaGetLastError: %d\n", ret, cudaGetLastError());
	end = std::chrono::steady_clock::now();
    duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    printf("Elapsed time for GPU 1 RDMA in milliseconds: %li ms.d\n\n", duration);
...
__global__ void mvt_kernel1_rdma_2gpu(rdma_buf<DATA_TYPE> *a, DATA_TYPE *x1, DATA_TYPE *y_1, int gpu)
{
	size_t i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void mvt_kernel2_rdma_2gpu(rdma_buf<DATA_TYPE> *a, DATA_TYPE *x2, DATA_TYPE *y_2, DATA_TYPE *a_gpu, int gpu)
{
	size_t i = blockIdx.x * blockDim.x + threadIdx.x;
...
void mvtCuda_rdma(DATA_TYPE* a, DATA_TYPE* &x1, DATA_TYPE* &x2, DATA_TYPE* y_1, DATA_TYPE* y_2)
{
    ...
    // transfer<<<2048, 512>>>(rdma_a->size/sizeof(DATA_TYPE), rdma_a);
    // cudaSetDevice(0);
    cudaError_t ret = ( cudaError_t) 0; // = cudaDeviceSynchronize();
    // check<<<2048, 512>>>(rdma_a->size/sizeof(DATA_TYPE), rdma_a, a_gpu);
    printf("ret: %d cudaGetLastError: %d for transfer\n", ret, cudaGetLastError());
    ...
    cudaSetDevice(0);
	mvt_kernel1_rdma_2gpu<<<grid, block, 0, stream0>>>(rdma_a_gpu0, x1_gpu, y_1_gpu, 0);
    mvt_kernel2_rdma_2gpu<<<grid, block, 0, stream0>>>(rdma_a_gpu0, x2_gpu, y_2_gpu, y_2_gpu, 0);

    cudaSetDevice(1);
	mvt_kernel1_rdma_2gpu<<<grid, block, 0, stream1 >>>(rdma_a_gpu1, x1_gpu1, y_1_gpu1, 1);
    mvt_kernel2_rdma_2gpu<<< grid, block, 0, stream1 >>>(rdma_a_gpu1, x2_gpu1, y_2_gpu1, y_2_gpu1, 1);
    ...
    printf("Elapsed time for GPUs RDMA in milliseconds: %li ms.d\n\n", duration);
...
{   
    ...
    if(rdma_flag){
        ...
        // mvtCuda_rdma_original(a, x1, x2, y_1, y_2);
        cudaSetDevice(0);
    ...
    mvtCuda_2gpu(a, x1_uvm, x2_uvm, y_1, y_2);
