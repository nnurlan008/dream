...
#include "../../include/runtime_prefetching_2gpu_2nic.h"
...
__global__ void 
kernel_coalesce_new_repr_rdma(uint level, size_t n, size_t numVertex, const uint64_t new_size, unsigned int *d_distance, 
             ...
             unsigned int *u_adjacencyList, unsigned int *u_edgesOffset, int u_case) 
{
    ...
    unsigned int *d_distance_gpu0, *d_edgesSize_gpu0, *d_edgesOffset_gpu0, *d_adjacencyList_gpu0, *d_startVertices_gpu0, *d_vertexList_gpu0;
    unsigned int *d_new_vertexList_gpu0, *d_new_offset_gpu0;
    unsigned int *return_distance;

    unsigned int *d_distance_gpu1, *d_edgesSize_gpu1, *d_edgesOffset_gpu1, *d_adjacencyList_gpu1, *d_startVertices_gpu1, *d_vertexList_gpu1;
    unsigned int *d_new_vertexList_gpu1, *d_new_offset_gpu1;

    unsigned int *uvm_adjacencyList_gpu0, *uvm_adjacencyList_gpu1;
    // checkError(cudaMemcpy(d_adjacencyList, u_adjacencyList, sizeof(rdma_buf<unsigned int>), cudaMemcpyHostToDevice));

    return_distance = new uint[G.numVertices];
    cudaSetDevice(0);
    checkError(cudaMalloc((void **) &d_distance_gpu0, G.numVertices*sizeof(unsigned int)));
    uint *h_distance = new uint[G.numVertices];
    for (size_t i = 0; i < G.numVertices; i++)
    {
        h_distance[i] = 100000;
    ...
    h_distance[startVertex] = 0;
    checkError(cudaMemcpy(d_distance_gpu0, h_distance, G.numVertices*sizeof(unsigned int), cudaMemcpyHostToDevice));
    ...
    cudaSetDevice(0);
    checkError(cudaMalloc((void **) &d_edgesSize_gpu0, G.numVertices*sizeof(unsigned int)));
    checkError(cudaMalloc((void **) &d_edgesOffset_gpu0, (G.numVertices+1)*sizeof(unsigned int)));
    checkError(cudaMemcpy(d_edgesOffset_gpu0, u_edgesOffset, (G.numVertices+1)*sizeof(unsigned int), cudaMemcpyHostToDevice));

    cudaSetDevice(1);
    checkError(cudaMalloc((void **) &d_edgesSize_gpu1, G.numVertices*sizeof(unsigned int)));
    checkError(cudaMalloc((void **) &d_edgesOffset_gpu1, (G.numVertices+1)*sizeof(unsigned int)));
    checkError(cudaMemcpy(d_edgesOffset_gpu1, u_edgesOffset, (G.numVertices+1)*sizeof(unsigned int), cudaMemcpyHostToDevice));

    rdma_buf<unsigned int> *rdma_adjacencyList_gpu0;
    rdma_buf<unsigned int> *rdma_adjacencyList_gpu1;

    if(u_case == 1 || u_case == 2){
        cudaSetDevice(0);
        checkError(cudaMallocManaged(&rdma_adjacencyList_gpu0, sizeof(rdma_buf<unsigned int>)));
        rdma_adjacencyList_gpu0->start(G.numEdges *sizeof(unsigned int), 0, NULL);
        for(size_t i = 0; i < G.numEdges; i++){
            rdma_adjacencyList_gpu0->local_buffer[i] = u_adjacencyList[i];
        }
        printf("line: %d rdma edge list done\n", __LINE__);
        cudaSetDevice(1);
        checkError(cudaMallocManaged(&rdma_adjacencyList_gpu1, sizeof(rdma_buf<unsigned int>)));
        rdma_adjacencyList_gpu1->start(G.numEdges *sizeof(unsigned int), 1, NULL);
        for(size_t i = 0; i < G.numEdges; i++){
            rdma_adjacencyList_gpu1->local_buffer[i] = u_adjacencyList[i];
        }
    }
    ...
    if(u_case == 2 || u_case == 3 || u_case == 4){
        cudaSetDevice(0);
        checkError(cudaMalloc((void **) &d_new_vertexList_gpu0, new_size*sizeof(unsigned int)));
        checkError(cudaMemcpy(d_new_vertexList_gpu0, new_vertex_list, new_size*sizeof(unsigned int), cudaMemcpyHostToDevice));
        
        checkError(cudaMalloc((void **) &d_new_offset_gpu0, (new_size+1)*sizeof(unsigned int)));
        checkError(cudaMemcpy(d_new_offset_gpu0, new_offset, (new_size+1)*sizeof(unsigned int), cudaMemcpyHostToDevice));

        cudaSetDevice(1);
        checkError(cudaMalloc((void **) &d_new_vertexList_gpu1, new_size*sizeof(unsigned int)));
        checkError(cudaMemcpy(d_new_vertexList_gpu1, new_vertex_list, new_size*sizeof(unsigned int), cudaMemcpyHostToDevice));
        
        checkError(cudaMalloc((void **) &d_new_offset_gpu1, (new_size+1)*sizeof(unsigned int)));
        checkError(cudaMemcpy(d_new_offset_gpu1, new_offset, (new_size+1)*sizeof(unsigned int), cudaMemcpyHostToDevice));
    }
    
    unsigned int changed_h_gpu0, changed_h_gpu1, *d_changed_gpu0, *d_changed_gpu1;
    cudaSetDevice(0);
    checkError(cudaMalloc((void **) &d_changed_gpu0, sizeof(unsigned int)));

    cudaSetDevice(1);
    checkError(cudaMalloc((void **) &d_changed_gpu1, sizeof(unsigned int)));
    ...
    ret1 = cudaDeviceSynchronize();
    if(u_case == 0 || u_case == 3){ // direct tranfser



        // Create CUDA events and streams
        cudaEvent_t event1_gpu0, event2_gpu0, event1_gpu1, event2_gpu1;
        cudaStream_t stream_gpu0, stream_gpu1;

        cudaSetDevice(0);
        cudaEventCreate(&event1_gpu0);
        cudaEventCreate(&event2_gpu0);
        cudaStreamCreate(&stream_gpu0);

        cudaSetDevice(1);
        cudaEventCreate(&event1_gpu1);
        cudaEventCreate(&event2_gpu1);
        cudaStreamCreate(&stream_gpu1);

        // Allocate memory on GPU 0
        cudaSetDevice(0);
        checkError(cudaMalloc((void **)&d_adjacencyList_gpu0, G.numEdges * sizeof(unsigned int)));

        // Allocate memory on GPU 1
        cudaSetDevice(1);
        checkError(cudaMalloc((void **)&d_adjacencyList_gpu1, G.numEdges * sizeof(unsigned int)));

        auto start = std::chrono::steady_clock::now();
        // Perform memory transfer to GPU 0 asynchronously
        cudaSetDevice(0);
        cudaEventRecord(event1_gpu0, stream_gpu0);
        checkError(cudaMemcpyAsync(d_adjacencyList_gpu0, u_adjacencyList, G.numEdges * sizeof(unsigned int), cudaMemcpyHostToDevice, stream_gpu0));
        cudaEventRecord(event2_gpu0, stream_gpu0);

        // Perform memory transfer to GPU 1 asynchronously
        cudaSetDevice(1);
        cudaEventRecord(event1_gpu1, stream_gpu1);
        checkError(cudaMemcpyAsync(d_adjacencyList_gpu1, u_adjacencyList, G.numEdges * sizeof(unsigned int), cudaMemcpyHostToDevice, stream_gpu1));
        cudaEventRecord(event2_gpu1, stream_gpu1);

        // Synchronize streams to wait for the transfers to complete
        cudaSetDevice(0);
        cudaStreamSynchronize(stream_gpu0);

        cudaSetDevice(1);
        cudaStreamSynchronize(stream_gpu1);

        // Calculate elapsed time for GPU 0 transfer
        float dt_ms_gpu0;
        cudaEventElapsedTime(&dt_ms_gpu0, event1_gpu0, event2_gpu0);
        printf("Elapsed time for transfer to GPU 0: %f ms\n", dt_ms_gpu0);

        // Calculate elapsed time for GPU 1 transfer
        float dt_ms_gpu1;
        cudaEventElapsedTime(&dt_ms_gpu1, event1_gpu1, event2_gpu1);
        printf("Elapsed time for transfer to GPU 1: %f ms\n", dt_ms_gpu1);

        auto end = std::chrono::steady_clock::now();
        long duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
        printf("Elapsed time in milliseconds for transfer to GPU 0 : %li ms. data: %f\n", duration, (double) G.numEdges*sizeof(unsigned int)/(1024*1024*1024llu));

        // Clean up
        cudaSetDevice(0);
        cudaEventDestroy(event1_gpu0);
        cudaEventDestroy(event2_gpu0);
        cudaStreamDestroy(stream_gpu0);

        cudaSetDevice(1);
        cudaEventDestroy(event1_gpu1);
        cudaEventDestroy(event2_gpu1);
        ...
        // auto start = std::chrono::steady_clock::now();
        // cudaEventRecord(event1, (cudaStream_t)1);
        // checkError(cudaMemcpy(d_adjacencyList_gpu0, u_adjacencyList, G.numEdges*sizeof(unsigned int), cudaMemcpyHostToDevice));
        // cudaEventRecord(event2, (cudaStream_t) 1);
        // auto end = std::chrono::steady_clock::now();
        ...
        // start = std::chrono::steady_clock::now();
        // cudaEventRecord(event1, (cudaStream_t)1);
        // checkError(cudaMemcpy(d_adjacencyList_gpu1, u_adjacencyList, G.numEdges*sizeof(unsigned int), cudaMemcpyHostToDevice));
        // cudaEventRecord(event2, (cudaStream_t) 1);
        // end = std::chrono::steady_clock::now();
        // cudaEventSynchronize(event1); //optional
        // cudaEventSynchronize(event2); //wait for the event to be executed!
        ...
        // printf("Elapsed time for transfer: %f\n", dt_ms);
        // duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
        // printf("Elapsed time in milliseconds for transfer to GPU 1: %li ms. data: %f\n", duration, (double) G.numEdges*sizeof(unsigned int)/(1024*1024*1024llu));
        // // exit(0);
    }
    ...
    unsigned int level_gpu0, level_gpu1;
    level_gpu0 = 0;
    level_gpu1 = 0;
    ...
    if(u_case == 6){
        ...
        checkError(cudaMallocManaged((void **) &uvm_adjacencyList_gpu0, G.numEdges*sizeof(unsigned int)));

        for(size_t i = 0; i < G.numEdges; i++){
            uvm_adjacencyList_gpu0[i] = G.adjacencyList_r[i];
        }
    ...
    // Create CUDA streams for GPU 0 and GPU 1
    cudaStream_t stream0, stream1;
    
    // Set device to GPU 0 and create stream
    cudaSetDevice(0);
    checkError(cudaStreamCreate(&stream0));

    // Set device to GPU 1 and create stream
    cudaSetDevice(1);
    checkError(cudaStreamCreate(&stream1));

    ret1 = cudaDeviceSynchronize();
    cudaEventRecord(event1, (cudaStream_t)1);

    while (changed_h_gpu0) {
        changed_h_gpu0 = 0;
        cudaSetDevice(0);
        checkError(cudaMemcpy(d_changed_gpu0, &changed_h_gpu0, sizeof(unsigned int), cudaMemcpyHostToDevice));
        cudaSetDevice(1);
        checkError(cudaMemcpy(d_changed_gpu1, &changed_h_gpu0, sizeof(unsigned int), cudaMemcpyHostToDevice));
        switch (u_case)
        {
        ...
        case 0:{
            // printf("direct transfer\n");
            cudaSetDevice(0);
            simpleBfs_modVertexList<<<G.numVertices / 1024 + 1, 1024, 0, stream0>>>
            (G.numVertices, level_gpu0, d_adjacencyList_gpu0, d_edgesOffset_gpu0, d_distance_gpu0, d_changed_gpu0); 
            cudaSetDevice(1);
        ...
        case 1:{
            ...
            size_t n_pages = G.numVertices*sizeof(unsigned int)/(4*1024)+1;
            cudaSetDevice(0);
            simpleBfs_rdma_optimized_warp<<</*G.numVertices / 256 + 1, 256*/(n_pages*32)/384 + 1, 384, 0, stream0>>>(
                        n_pages, G.numVertices, level_gpu0, rdma_adjacencyList_gpu0, d_edgesOffset_gpu0, d_edgesSize_gpu0, d_distance_gpu0, d_changed_gpu0); 
            cudaSetDevice(1);
        ...
        case 3:{
            ...
            kernel_coalesce_new_repr<<<(n_pages*32)/512 + 1, 512, 0, stream0>>>
            (level_gpu0, n_pages, G.numVertices, new_size, d_distance_gpu0, d_new_offset_gpu0, d_new_vertexList_gpu0, d_adjacencyList_gpu0,
            d_changed_gpu0);
            cudaSetDevice(1);
        ...
        case 4: {
            ...
            size_t n_pages = new_size*sizeof(unsigned int)/(4*1024)+1;
            kernel_coalesce_new_repr<<<(n_pages*32)/512 + 1, 512, 0, stream0>>>
            (level_gpu0, n_pages, G.numVertices, new_size, d_distance_gpu0, d_new_offset_gpu0, d_new_vertexList_gpu0, uvm_adjacencyList_gpu0,
            d_changed_gpu0);
            cudaSetDevice(1);
        ...
        case 5:{
            ...
            numblocks = ((G.numVertices * (WARP_SIZE / CHUNK_SIZE) + numthreads) / numthreads);
            dim3 blockDim(BLOCK_SIZE, (numblocks+BLOCK_SIZE)/BLOCK_SIZE);
            bfs_kernel_coalesce_chunk<<<blockDim, numthreads, 0, stream0>>>
            (d_distance_gpu0, level_gpu0, G.numVertices, d_edgesOffset_gpu0, uvm_adjacencyList_gpu0, d_changed_gpu0);
        ...
        case 6:{
            // printf("direct transfer\n");
            simpleBfs_modVertexList<<<G.numVertices / 1024 + 1, 1024, 0, stream0>>>
            (G.numVertices, level_gpu0, /*d_adjacencyList*/uvm_adjacencyList_gpu0, d_edgesOffset_gpu0, d_distance_gpu0, d_changed_gpu0); 
            cudaSetDevice(1);
            simpleBfs_modVertexList<<<G.numVertices / 1024 + 1, 1024, 0, stream1>>>
        ...
        // printf("ret1: %d cudaGetLastError(): %d level_gpu0: %d\n", ret1, cudaGetLastError(), level_gpu0);
        
        checkError(cudaMemcpy(&changed_h_gpu0, d_changed_gpu0, sizeof(unsigned int), cudaMemcpyDeviceToHost));
        level_gpu0++;
        level_gpu1++;
        cudaSetDevice(0);
        checkError(cudaStreamSynchronize(stream0));
    ...
    checkError(cudaMemcpy(return_distance, d_distance_gpu1, G.numVertices*sizeof(unsigned int), cudaMemcpyDeviceToHost));

    if(u_case == 2 || u_case == 3 || u_case == 4){
        cudaSetDevice(0);
        checkError(cudaFree(d_new_offset_gpu0));
        checkError(cudaFree(d_new_vertexList_gpu0));
        cudaSetDevice(1);
        checkError(cudaFree(d_new_offset_gpu1));
        checkError(cudaFree(d_new_vertexList_gpu1));
    }

    if(u_case == 0 || u_case == 3){ // direct transfer
        cudaSetDevice(0);
        checkError(cudaFree(d_adjacencyList_gpu0));
        cudaSetDevice(1);
        checkError(cudaFree(d_adjacencyList_gpu1));
    }
    if(u_case == 4 || u_case == 5 || u_case == 5){
        cudaFree(uvm_adjacencyList_gpu0);
        cudaFree(uvm_adjacencyList_gpu0);
    }


    cudaSetDevice(0);
    checkError(cudaFree(d_distance_gpu0));
    checkError(cudaFree(d_changed_gpu0));
    checkError(cudaFree(d_edgesOffset_gpu0));
    checkError(cudaFree(d_edgesSize_gpu0));

    cudaSetDevice(1);
    checkError(cudaFree(d_distance_gpu1));
    checkError(cudaFree(d_changed_gpu1));
    checkError(cudaFree(d_edgesOffset_gpu1));
    checkError(cudaFree(d_edgesSize_gpu1));
...
{   
    ...
    bool rdma_flag = true;
    cudaError_t ret1;
    struct context_2gpu_2nic *s_ctx = (struct context_2gpu_2nic *)malloc(sizeof(struct context_2gpu_2nic));
    if(rdma_flag){
        s_ctx->gpu_cq = NULL;
        s_ctx->wqbuf = NULL;
        s_ctx->cqbuf = NULL;
        ...
        s_ctx->gpu_buf_size[0] = 16*1024*1024*1024llu; // N*sizeof(int)*3llu;
        s_ctx->gpu_buf_size[1] = 16*1024*1024*1024llu; // N*sizeof(int)*3llu;
        ...
        int ret = local_connect_2gpu_2nic(mlx_name, s_ctx, 0);

        mlx_name = "mlx5_3";
        // int ret = local_connect(mlx_name, s_ctx);
        ret = local_connect_2gpu_2nic(mlx_name, s_ctx, 1);

        ret = prepare_post_poll_content_2gpu_2nic(s_ctx, &post_cont, &poll_cont, &post_cont2, \
    ...
    if(rdma_flag){
        ...
        cudaSetDevice(0);
        cudaFree(s_ctx->gpu_buffer[0]);
        cudaSetDevice(1);
        cudaFree(s_ctx->gpu_buffer[1]);
    }
...
void simpleBfs_rdma(size_t n, unsigned int level, rdma_buf<unsigned int> *d_adjacencyList, unsigned int *d_edgesOffset,
    ...
    if(thid < n /*d_distance->size/sizeof(uint)*/){
        ...
        if (k == level) {
            ...
            for (size_t i = d_edgesOffset[thid]; i < d_edgesOffset[thid+1]; i++) {
                unsigned int v;
...
__global__ void
selectmin_distance(unsigned int *d_distance1, unsigned int *d_distance2, size_t numVertex){
    // Global thread ID
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if(tid < numVertex && (d_distance1[tid] < d_distance2[tid]))
        d_distance1[tid] = d_distance2[tid];
}

__global__ void kernel_coalesce_new_repr_rdma(uint level, size_t n, size_t gpu, const uint64_t new_size, unsigned int *d_distance, 
                                unsigned int *new_offset, unsigned int *new_vertex_list, rdma_buf<unsigned int> *edgeList, unsigned int *changed) {
    ...
    // Ensure we don't process out-of-bounds pages
    if (pageStart < n * pageSize /*&& pageStart >= numVertex*n*(pageSize/2)*/  ) {
