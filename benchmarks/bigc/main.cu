...
void bicgCuda()
{
    ...
    printf("CPU Runtime in milliseconds: %li ms.d\n\n", duration);
...
void bicgRDMA()
{
    ...
    A_rdma->start(NX*NY*sizeof(DATA_TYPE));
    ...
    cudaDeviceSynchronize();
    auto start = std::chrono::steady_clock::now();
	bicg_kernel1_rdma<<< grid1, block >>>(A_rdma, r_gpu, s_gpu);
	ret = cudaDeviceSynchronize();
    printf("ret: %d cudaGetLastError: %d\n", ret, cudaGetLastError());
	bicg_kernel2_rdma<<< grid2, block >>>(A_rdma, p_gpu, q_gpu);
	printf("ret: %d cudaGetLastError: %d\n", ret, cudaGetLastError());
