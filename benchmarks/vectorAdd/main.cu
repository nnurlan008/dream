...
void add_uvm()
{
    ...
    printf("Allocating results for verification on CPU\n");
    h_result = (DATA_TYPE *) malloc(N*sizeof(DATA_TYPE));
    // for(size_t i = 0; i < N; i++) 
    //     h_result[i] = 4;

    printf("Allocating results for verification on GPU\n");
	// check_cuda_error(cudaMalloc(&a_gpu, sizeof(DATA_TYPE) * N * N));
	check_cuda_error(cudaMalloc(&d_result, sizeof(DATA_TYPE) * N));  
    check_cuda_error(cudaMallocManaged(&a_gpu, sizeof(DATA_TYPE) * N));  
    check_cuda_error(cudaMallocManaged(&b_gpu, sizeof(DATA_TYPE) * N));   
    ...
    check_cuda_error(cudaMemAdvise(a_gpu, sizeof(DATA_TYPE) * N, cudaMemAdviseSetReadMostly, 0));
    check_cuda_error(cudaMemAdvise(b_gpu, sizeof(DATA_TYPE) * N, cudaMemAdviseSetReadMostly, 0));
    // oversubs(99, sizeof(DATA_TYPE) * N * 3);
...
>>>>>>> origin/cloudlab
    ...
    // print_retires<<<1,1>>>();
    printf("Line: %d\n", __LINE__);
    check_cuda_error(cudaMemcpy(h_result, d_result, sizeof(DATA_TYPE) * N, cudaMemcpyDeviceToHost));  
    printf("Line: %d\n", __LINE__);
    for(size_t i = 0; i < N; i++) {
        if(h_result[i] != 4){
            printf("Error on %llu\n", i);
    ...
    check_cuda_error(cudaFree(a_gpu));
    check_cuda_error(cudaFree(b_gpu));
...
//     // Page size in elements (64KB / 4 bytes per unsigned int)
//     const size_t pageSize = 8*1024/8 / sizeof(unsigned int);
//     // Elements per warp
//     const size_t elementsPerWarp = pageSize / warpSize;
...
void add_rdma()
{
    ...
    rdma_a->start(3*N*sizeof(DATA_TYPE));

    for(size_t i = 0; i < 3*N; i++){
        rdma_a->local_buffer[i] = 2;
    }
