...
void readfile(Graph_x &G, Graph_m &G_m, int argc, char **argv, uint *u_edgesOffset, \
    ...
    string input_weights = "/mydata/weights_" + fileName;
    std::cout << "input_weights name: " << input_weights << std::endl;
    ...
    if (read_from_file == 1){
        ...
        infile.close();
        printf("Edgelist file closed!\n");
        printf("Reading weights...\n");

        u_weights = new uint[num_edges];
        std::ifstream infile (input_weights, ios::in | ios::binary);
        infile.read ((char*)u_weights, sizeof(uint)*num_edges);
        infile.close();
...
void readfile(Graph_x &G, Graph_m &G_m, int argc, char **argv, uint *u_edgesOffset, \
    ...
    if (read_from_file == 1){
        ...
        u_weights = new float[num_edges];
        std::ifstream infile (input_weights, ios::in | ios::binary);
        infile.read ((char*)u_weights, sizeof(float)*num_edges);
        infile.close();
