...
void readfile(Graph &G, Graph_m &G_m, int argc, char **argv, uint *u_edgesOffset, \
    ...
    std::ifstream infile (argv[7], ios::in | ios::binary);
    if(argc == 5){
        // fp = fopen(argv[4], "r");
        
        // std::ifstream infile (argv[4], ios::in | ios::binary);
        if (!infile.is_open()) {
            cerr << "Error opening file: " << argv[7] << endl;
            infile.close();
            // exit(-1);
            read_from_file = 0;
        }
    ...
    if (read_from_file == 1){
        ...
        std::cout << "Opening file: " << argv[7] << endl;
        ...
        G.edgesOffset_r = new uint[G.numVertices];
        G.edgesSize_r = new uint[G.numVertices];
        G.adjacencyList_r = new uint[G.numEdges];
		...
		infile.read ((char*)G.edgesOffset_r, sizeof(uint)*num_nodes);
