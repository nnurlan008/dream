...
__global__ void 
alloc_global_content(struct post_content *post_cont, struct poll_content *poll_cont, struct post_content2 *post_cont2,
...
struct rdma_buf {
    ...
    bool release_page(uint64_t page_n){
                ...
                do{
                    ...
                    if(page_to_evict != -1){
                        ...
                        while(atomicCAS((unsigned int *)entry, 0, 10000) != 0){
                            ...
                            //     printf("stuck in eviction for reading page: %d entry: %d\n", (int) che, (int) *entry);
                            // }
                            // retries++;
                        }
        ...
        void read(uint64_t che){
            ...
            int qp_index = get_smid()%84 + 84*gpu; // che%8; // (); // warp_id() % 256; // ;
            ...
            void *cqe = /*gpoll_cont.cq_buf*/ (void *) gpu_cq_bufs[gpu] + 2*4096*(qp_index-gpu*84) + (cur_post & 63) * 64;
            struct mlx5_cqe64 *cqe64 = (struct mlx5_cqe64 *) cqe;

            void *qp_buf = (void *) gpu_qp_bufs[gpu];
            uint32_t lkey = gpu_mrs_lkey[gpu];
            ...
            post_m(/*d_TLB[che].host_address*/ rem_addr, gpost_cont2.wr_rdma_rkey[rkey_index], data_size, lkey /*gpost_cont.wr_sg_lkey*/, dev_addr + pageNumber*request_size*sizeof(T)/*d_TLB[che].device_address*/, 4, gpost_cont.qp_num + qp_index, 
        ...
        T operator[](const size_t index) {
                ...
                T *tmp_array = (T *) (dev_addr + /*atomicAdd((int *)che_entry, 0)*/ page_number[che]*request_size*sizeof(T)); 
