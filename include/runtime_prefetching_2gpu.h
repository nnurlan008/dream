...
struct rdma_buf {
    ...
    void read_batch(uint64_t che){
        ...
        void read(uint64_t che){
            ...
            // printf("gpost_cont.qp_buf: %p\n", gpost_cont.qp_buf);
            // printf("gpost_cont.bf_reg[%d]: %p\n", (qp_index), gpost_cont.bf_reg[qp_index]);
            post_m(/*d_TLB[che].host_address*/ rem_addr, gpost_cont2.wr_rdma_rkey[rkey_index], data_size, lkey /*gpost_cont.wr_sg_lkey*/, dev_addr + che*request_size*sizeof(T)/*d_TLB[che].device_address*/, 4, gpost_cont.qp_num + qp_index, 
