...
struct rdma_buf {
    ...
    bool release_page(uint64_t page_n){
        ...
        void read(uint64_t che){
            ...
            // post_m(/*d_TLB[che].host_address*/ rem_addr, server_content.wr_rdma_rkey[rkey_index+warpId*5], data_size, lkey /*gpost_cont.wr_sg_lkey*/, dev_addr + pageNumber*request_size*sizeof(T)/*d_TLB[che].device_address*/, 4, gpu_qp_num[warpId] + (qp_index-128*warpId), 
