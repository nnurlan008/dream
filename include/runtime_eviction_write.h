...
#define oversubs_ratio_macro  100
#define rest_memory 12*1024*1024*1024llu // restricted memory
...
struct rdma_buf {
    ...
    bool release_page(uint64_t page_n){
        ...
        __forceinline__
        __device__
        void write(uint64_t cpu_number, uint64_t gpu_number){

            int qp_index = get_smid()%44+84; // che%8; // (); // warp_id() % 256; // ;
            ...
            while(atomicCAS((unsigned int *)cq_lock, 0, 1) != 0){
                ...
                if(retries >= 100000){
                    ...
                    printf("write - qp_index: %d retries: %d stuck in lock cq\n", qp_index, retries);
                    retries = -1;
                }
                retries++;
            }
            ...
            uint64_t rem_addr = host_address + cpu_number*request_size*sizeof(T);
            int rkey_index = (/*d_TLB[che].host_address*/ rem_addr - d_remote_address/*gpost_cont.wr_rdma_remote_addr*/)/(8*1024*1024*1024llu);
            uint64_t value_ctrl;
            // int finished;
            ...
            post_opt_write(rem_addr, gpost_cont2.wr_rdma_rkey[rkey_index],            
                    dev_addr + gpu_number*request_size*sizeof(T),
                    cur_post, qp_index);
            ...
            while(*op_flag == 240){
                    ...
                    retries++;
                    if(retries>100000) {
                        printf("write - stuck in completion che: %llu qp_index: %d\n", cpu_number, qp_index);
                        retries=-1;
                    }
            ...
            if(pageNumber == -1){
                do{
                    ...
                    if(page_to_evict != -1){
                        ...
                        if(d_tlb[page_to_evict] == 2)
                            atomicCAS(&d_tlb[page_to_evict], 2, 0);
                        else if(d_tlb[page_to_evict] == 3){
                            write(page_to_evict, pageNumber);
                            atomicCAS(&d_tlb[page_to_evict], 3, 0);
                        }
            ...
            uint64_t che;
                ...
                if(leader1 == lane_id()){
                    ...
                    do{
                        turn = atomicAdd((unsigned int *)entry, 1);
                        if(turn == 0){
                            if(d_tlb[che] == 0){ // page is on cpu
                                // atomicCAS(&d_tlb[che], 0, 1);
                                read(che);
                                // read_batch(che);
                                atomicCAS(&d_tlb[che], 0, 3);
                            }
                            break;
                        }
                        else if(turn > 0 && turn < 5000){
                ...
                // if(page_number[che] < 0 || page_number[che] >= num_pages)
                //     printf("tmp_array: %p page_number[che]: %d\n", tmp_array, page_number[che]);
                tmp_array[index&(request_size-1)] = new_value;
                // dev_buffer[index];
                __syncwarp(mask1);
                // __syncthreads();
                if(leader1 == lane_id()){
